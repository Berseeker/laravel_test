    ...
    public function index(Request $request)
    {
        ...
        if($user != NULL)
        {
            if(Hash::check($request->password,$user->password)){
                ...
                {
                    //dd($user->id);
                    $token = $user->createToken('login',['can:all']);
                }else if($user->rol == "supervisor")
                {
                    $token = $user->createToken('login',['can:all-slave']);
                }else if($user->rol == "client")
                {
                    $token = $user->createToken('login',['can:slave']);
                }
                return $this->loginResponse('Inicio de sesion exitoso',$user,$token->plainTextToken,200);
